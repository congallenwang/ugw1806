		...
		for radioname, radiostate in pairs(_wifi_state()) do
			...
			then
				...
				for netidx, netstate in ipairs(radiostate.interfaces) do
					...
					then
						utl.perror("setion=:"..netstate.section)
						local t2, n2 = _uci:get("wireless", netstate.section)
						utl.perror("t1:"..t1.." ,n1=:"..n1)
						utl.perror("t2:"..t2.." ,n2=:"..n2)
...
local function _wifi_iwinfo_by_ifname(ifname, force_phy_only)
	...
	--utl.perror("in _wifi_iwinfo_by_ifname, ifname="..ifname)
			...
			_uci:foreach("wireless", "wifi-iface",
				function(s)
					if s.device == radioname then
						...
						if i == netidx then
							n = s[".name"]
							utl.perror("_wifi_sid_by_netid:"..netid)
							return false
						end
					end
				end)

			utl.perror("_wifi_sid_by_netid return n:"..n)
			return n
		end
	end
end
...
function wifinet.__init__(self, name, data)
	...
	--Cong
	if netstate  then		
		netstate.ifname=netstate.config.ifname
	end

	--utl.perror("wifinet init, ifname: "..netstate.ifname)
